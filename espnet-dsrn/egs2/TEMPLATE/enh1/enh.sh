...
python=          # Specify python to execute espnet commands
device=
...
# inference_asr_tag长度为0则为真
if [ -z "${inference_asr_tag}" ]; then
    # inference_asr_config 非0则为真
...
enh_stats_dir="${expdir}/enh_stats_raw_char"
...
if ! "${skip_train}"; then
    ...
    if [ ${stage} -le 6 ] && [ ${stop_stage} -ge 6 ]; then
        ...
        # prepare train and valid data parameters
        _train_data_param="--train_data_path_and_name_and_type ${_enh_train_dir}/wav.scp,speech,${_type} "
        _train_data_param+="--train_data_path_and_name_and_type ${_enh_train_dir}/clean.scp,speech_ref1,${_type} "
        _train_shape_param="--train_shape_file ${enh_stats_dir}/train/speech_shape "
        _train_shape_param+="--train_shape_file ${enh_stats_dir}/train/speech_ref1_shape "
        _valid_data_param="--valid_data_path_and_name_and_type ${_enh_valid_dir}/wav.scp,speech,${_type} "
        _valid_data_param+="--valid_data_path_and_name_and_type ${_enh_valid_dir}/clean.scp,speech_ref1,${_type} "
        _valid_shape_param="--valid_shape_file ${enh_stats_dir}/valid/speech_shape "
        _valid_shape_param+="--valid_shape_file ${enh_stats_dir}/valid/speech_ref1_shape "
