...
def mask_along_axis(
    ...
    B = spec.shape[0]
    # D = Length or Freq
    # 在D上施加mask,这里在frequency上施加，D的值为257
    D = spec.shape[dim]
    # mask_length: (B, num_mask, 1)
    # torch.randint()返回一个张量，张量的值为整数，范围为[0, 30),形状为(B, num_mask)，unsqueeze(2)在第三个维度前加一个维度
    # mask_length (B, num_mask, 1)
    ...
    # mask的位置
    # mask_length.max()返回张量中元素的最大值,位置的最大值必须小于mask长度的最大值
