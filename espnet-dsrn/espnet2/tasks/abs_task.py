...
class AbsTask(ABC):
    ...
    def build_iter_options(
        ...
        # 如果是训练模式就令data_path_and_name_and_type等于train_data_path_and_name_and_type，然后创建Dataset
        # collate_fn打包函数，在具体任务中实现
        if mode == "train":
    ...
    def build_sequence_iter_factory(
        ...
        # 返回上级目录路径，并判断是否存在utt2category文件
        if Path(
        ...
        # 构建迭代器，根据dataset和采样器来构建一个torch的dataloader
    ...
    def build_multiple_iter_factory(
        ...
        # 1. Sanity check
        #  iter_options.data_path_and_name_and_type = args.train_data_path_and_name_and_type
        num_splits = None
        for path in [
    ...
    def build_streaming_iterator(
        ...
        cls.check_task_requirements(
            dataset, allow_variable_data_keys, train=train, inference=inference
        )
