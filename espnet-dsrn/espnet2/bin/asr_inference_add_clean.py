...
class Speech2Text:
    ...
    # 定义__call__方法,那么类的实例化对象就是可调用对象,可以在实例化对象后面加()来调用，调用时执行的代码就是方法__call__中定义的代码。
    # 使得类实例对象可以像调用普通函数那样，以“对象名()”的形式使用。
    @torch.no_grad()
    def __call__(
        self, speech: Union[torch.Tensor, np.ndarray], clean: Union[torch.Tensor, np.ndarray]
    ) -> List[
        Tuple[
            Optional[str],
            List[str],
        ...
        clean = clean.unsqueeze(0).to(getattr(torch, self.dtype))

        # lengths: (1,)
        lengths = speech.new_full([1], dtype=torch.long, fill_value=speech.size(1))
        clean_lengths = clean.new_full([1], dtype=torch.long, fill_value=clean.size(1))
        
        batch = {"speech": speech, "speech_lengths": lengths, "clean": clean, "clean_lengths": clean_lengths}
...
def inference(
    ...
    with DatadirWriter(output_dir) as writer:
        for keys, batch in loader:
            ...
            # all()，返回可迭代对象中所有元素为true则返回true，否则返回false
            # 检查keys是否全是字符串对象
