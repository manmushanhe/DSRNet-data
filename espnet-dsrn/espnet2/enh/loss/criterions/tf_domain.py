...
def _create_mask_label(mix_spec, ref_spec, noise_spec=None, mask_type="IAM"):
    ...
    for idx, r in enumerate(ref_spec):
        ...
        if mask_type == "IBM":
            ...
            # mask = np.abs(clean_S) / (np.abs(noisy_S) + _eps)
            # ref_spec参考语谱图即是干净语音的语谱图
    ...
    if mask_type == "IAM":
            # mask = np.abs(clean_S) / (np.abs(noisy_S) + _eps)
            # ref_spec参考语谱图即是干净语音的语谱图
        mask = abs(ref_spec) / (abs(mix_spec) + EPS)
        mask = mask.clamp(min=0, max=1)
    elif mask_type == "IRM":
        beta = 0.5
        mask = (abs(ref_spec).pow(2) / (abs(mix_spec).pow(2) + EPS)).pow(beta)
    return mask
...
class FrequencyDomainMSE(FrequencyDomainLoss):
    def __init__(self, compute_on_mask=True, mask_type="IAM", name=None):
        super().__init__()
    ...
    def forward(self, ref, inf) -> torch.Tensor:
        ...
        # 再求每一维误差的平均
        if ref.dim() == 3:
            # mean 在后两个维度T,F维度上做平均返回值得size为(Batch,)
