...
class Dsrn_two_branch(torch.nn.Module):
    def __init__(self, use_adaptive_weight: bool = False):
        ...
        self.bn = torch.nn.BatchNorm1d(1,eps=1e-9,momentum=0.001,affine=False,track_running_stats=True)
    ...
    def forward(self, enhanced: torch.Tensor, noise: torch.Tensor):
        ...
        '''
        ratio_current = ratio.cpu().detach().numpy()
        with open("/Work21/2021/luhaoyu/espnet/egs2/aishell_noise/enh1/ratio.csv","a") as csvfile:
            writer = csv.writer(csvfile)
            writer.writerows(ratio_current)
        data = pd.read_csv("/Work21/2021/luhaoyu/espnet/egs2/aishell_noise/enh1/ratio.csv")
        a = data.describe()
        mid = a.iloc[5,0]
        '''
