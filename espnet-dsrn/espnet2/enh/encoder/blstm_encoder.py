...
class LSTMEncoder(AbsEncoder):
    ...
    def __init__(
        ...
        super().__init__()
        #self.lstm = nn.LSTM(input_size=input_size, hidden_size=hidden_size, num_layers=num_layers, batch_first=True, dropout=0.4, bidirectional=True)
        #self.linear = nn.Linear(in_features=hidden_size*2, out_features=input_size)
