import torch
from espnet2.enh.encoder.Only_Noisy_Training_main.preprocess import TorchSignalToFrames
...
class mse_loss(object):
    def __call__(self, outputs, labels, loss_mask):
        #print(outputs.shape)  # torch.Size([5, 1, 50432])
        #print(loss_mask.shape)  # torch.Size([5, 1, 50624])
...
class stftm_loss(object):
    def __init__(self, frame_size=512, frame_shift=256, loss_type='mae'):
        ...
        DR = np.real(D)
        DI = np.imag(D)
        self.DR = torch.from_numpy(DR).float().to(self.device) #cuda()  # to(self.device)
