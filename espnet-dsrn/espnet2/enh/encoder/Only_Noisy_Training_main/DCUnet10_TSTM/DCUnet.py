...
class DCUnet10(AbsEncoder):
    ...
    def __init__(
        ...
        self.lstm1 = nn.LSTM(input_size=512, hidden_size=1024, num_layers=2, batch_first=True, dropout=0.4)
        #,bidirectional=True
        self.linear1 = nn.Linear(in_features=1024, out_features=512)
        
        self.lstm2 = nn.LSTM(input_size=512, hidden_size=1024, num_layers=2, batch_first=True, dropout=0.4)
        #,bidirectional=True
    ...
    def forward(self, x):
        ...
        x_real = x[..., 0]
        # print("x_real",x_real.shape) torch.Size([5, 512, 198])
        x_im = x[..., 1]

        x_real = x_real.transpose(2,1) # torch.Size([5, 198, 512])   
        x_im = x_im.transpose(2,1)     # torch.Size([5, 198, 512])   

        output_real, h = self.lstm1(x_real)
        output_real = self.linear1(output_real)
        output_real = self.activation(output_real)
        
        output_im, h = self.lstm2(x_im)
        #print(x_im.shape)
        #print(output_im.shape)
        output_im = self.linear2(output_im)
        output_im = self.activation(output_im)
        
        mask = torch.stack([output_real, output_im], dim=-1)  # mask: torch.Size([5, 198, 512, 2])
        mask = mask.transpose(2,1)  # mask: torch.Size([5, 512, 198, 2])

        output = x * mask    # u4 - the mask
                             # u4: torch.Size([5, 512, 198, 2])
